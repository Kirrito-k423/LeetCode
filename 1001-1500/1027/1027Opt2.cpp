...
public:
    int longestArithSeqLength(vector<int>& nums) {
		...
		int deltaJ2k[501];
		memset(deltaJ2k,-1,sizeof(deltaJ2k));
		for(int i=1; i<n ;i++){
			deltaJ2k[nums[i-1]]=i-1;
			for(int j = 0; j <= 2*maxJ; j++){
				int targetVal = nums[i]-(j-maxJ);
				if(targetVal >=0 && targetVal<=500 && deltaJ2k[targetVal]!=-1){
					// 结果单增，所以选择最后一个
					dp[i][j] += dp[deltaJ2k[targetVal]][j];
